...
main_application.title("Moon Text Editor")
...
font_color_picker_icon=tk.PhotoImage(file="icons2/font_color.png")
font_color_picker_btn=ttk.Button(tool_bar,image=font_color_picker_icon)
font_color_picker_btn.grid(row=0,column=5,padx=5)
...
font_color_picker_btn.configure(command=change_font_color)
...
# Status bar checking for words and characters
...
def save_as_file(event=None):
    ...
    try:
        ...
        url=url=filedialog.asksaveasfile(mode="w",defaultextension='.txt',filetypes=(("Text Files",'*.txt'),('All Files','*.*')))
        url.write(content)
        url.close()
    except:
        return
...
def exit_file(event=None):
    ...
    try:
        if text_changed:
            m_box=messagebox.askyesnocancel("File not saved","Do you want to save the file?")
            if m_box is True:
                if url:
                    content=text_editor.get(1.0,'end')
...
# Clear all command
edit.add_command(label='Clear All',image=clear_all_icon,compound=tk.LEFT,accelerator="Ctrl+Alt+C"
